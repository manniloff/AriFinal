...
//				.setPriority(Thread.MAX_PRIORITY).build());
	...
	public boolean stopSmppClient() {
		...
		if(clearDLRList != null 
				&& !clearDLRList.isInterrupted()) {
			logger.warn("force stop clearDLRList");
			clearDLRList.stop();
		}
		if(clearSMParts != null
				&& !clearSMParts.isInterrupted()) {
			logger.warn("force stop clearSMParts");
			clearSMParts.stop();	
		}
		if(resetCounter != null
				&& !resetCounter.isInterrupted()) {
			logger.warn("force stop resetCounter");
			resetCounter.stop();
		}
		if(getMTSMSToSend != null
				&& !getMTSMSToSend.isInterrupted()) {
			logger.warn("force stop getMTSMSToSend");
			getMTSMSToSend.stop();
		}
		exec.shutdown();
		esSendSM.shutdown();
//		esProcessRequset.shutdown();
	...
	public static boolean sendSM(SendData smsData) {
		SmppClient alternativeClient = getAvailableSMPPConncetion();
		if(alternativeClient != null) {
			int t = alternativeClient.getConLimit().counter.get();
