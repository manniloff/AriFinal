...
//			logger.warn("MoveToSMSData is too long MoveToSMSDataLimit - " + saveResponseTimeLimit);
	...
	public void saveSRISMResponse(SendData data, String state, String errorMessage, boolean numberNotInRoaming
		...
		try {
			session.beginTransaction();
			session.createSQLQuery(
				  "INSERT srism_response(messageId, dialogId, errorCode, errorMessage, receivedResponse, mscAddress) "
				+ "VALUES(:messageId, :dialogId, :errorCode, :errorMessage, :receivedResponse, :mscAddress);")
			...
			.setParameter("receivedResponse", new Timestamp(started))
			.setParameter("mscAddress", (!numberNotInRoaming) 
					? mscAddress : null).executeUpdate();
			if(state == null) {
...
//			logger.warn("processing is too long! RequsetTimeLimit = " + requsetTimeLimit);
	...
	public boolean processCancelSmRequest(long messageId, AccessType reqType, int accessId, String queueType) {
		Session session = getSession();
		boolean result = false;
		try {
			session.beginTransaction();
			result = (boolean)session.getNamedQuery("addCancelSMRequest")
					...
					.setParameter("reqType", reqType.name())
					.setParameter("accessId", accessId)
					.setParameter("queueType", queueType).uniqueResult();
			session.getTransaction().commit();
		} catch (Exception e) {
			if (session.getTransaction() != null)
				session.getTransaction().rollback();
